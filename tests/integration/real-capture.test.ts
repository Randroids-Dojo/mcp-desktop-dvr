...
describe('Real Screen Capture Integration Tests', () => {
  ...
  describe('Screen Recording Permissions', () => {
    it('should prompt for screen recording permission and capture desktop', async () => {
      console.log('\n📹 This test requires Screen Recording permission.');
      console.log('If prompted, please grant permission in System Preferences > Privacy & Security > Screen Recording\n');
  ...
  describe('Visual Analysis', () => {
    it('should capture and analyze desktop activity', async () => {
      ...
      console.log(`App Context: ${analysis.appContext || 'Not detected'}`);
      console.log(`Errors found: ${analysis.errors.length}`);
      console.log(`Warnings found: ${analysis.warnings.length}`);
      console.log(`Click events: ${analysis.clickAnalysis.length}`);
  ...
  describe('Performance Under Load', () => {
    it('should handle continuous capture for extended period', async () => {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        console.log(`${i * 5 + 5}s - Buffer segments: ${status.bufferStatus.totalSegments}, Memory: ${currentMemory.toFixed(1)}MB`);
      }
